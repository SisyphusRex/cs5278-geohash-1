...
public class GeoHash {
     ...
     *
     * This class uses a main() method where we can write our own simple
     * "experiments" to
     * test how our code works. You are encouraged to modify the main method to play
     * around
     * with your code and understand it. When you have working code, you can extract
     * it
     * into a method. When you have working examples with assertions, you can
     * extract them
     * into tests.
     *
     * I have left some sample experiments in main() to help you understand the
     * geohash
     * algorithm.
     *
     *
    ...
    public static boolean[] geohash1D(double valueToHash, double[] valueRange, int bitsOfPrecision) {
        ...
        double[] newValueRange = valueRange.clone();
        bitsOfPrecision -= 1;
        double midpoint = (newValueRange[0] + newValueRange[1]) / 2;
        // System.out.println(String.format("value: %f range: %f %f midpoint: %f",
        // valueToHash, newValueRange[0],
        // newValueRange[1], midpoint));
        if (valueToHash >= midpoint) {
            boolean bit = true;
            geohash[0] = bit;
            newValueRange[0] = midpoint;
            boolean[] subGeoHash = geohash1D(valueToHash, newValueRange, bitsOfPrecision);
            if (subGeoHash != null) {
                for (int i = 0; i < subGeoHash.length; i++) {
                    geohash[i + 1] = subGeoHash[i];
                }
            }
        ...
        // String hashArrayString = String.format("%d ", bitsOfPrecision);
        // for (int i = 0; i < geohash.length; i++) {
        // hashArrayString += geohash[i];
        // }
        // System.out.println(hashArrayString);
    ...
    public static boolean[] geohash2D(double v1, double[] v1range, double v2, double[] v2range, int bitsOfPrecision) {
        ...
        // @ToDo:
        //
        // Separately compute individual 1D geohashes for v1 and v2 and then interleave
        // them together
    ...
    public static String toHashString(boolean[] geohash) {
        ...
        for (boolean b : geohash) {
            hashString += (b ? "1" : "0"); // if b is true, return 1, if false, return 0
        }
        return hashString;
    }
    ...
    public static void main(String[] args) {
        ...
        // If you can get the 1D geohash to pass all of these faux tests, you should be
        // in
